...
from jiutian.mm_utils import expand2square
...
conv_llama_3_2 = Conversation(
    ...
    tokenizer_id="/data1/Models/Llama-3.2-3B-Instruct",
    tokenizer=safe_load_tokenizer("/data1/Models/Llama-3.2-3B-Instruct"),
...
conv_llama_3_1 = Conversation(
    ...
    sep="<|eot_id|>",
    sep_style=SeparatorStyle.LLAMA_3,
    tokenizer_id="/data1/Models/Llama-3.1-8B-Instruct",
    tokenizer=safe_load_tokenizer("/data1/Models/Llama-3.1-8B-Instruct"),
    stop_token_ids=[128009],
)


conv_llama_3 = Conversation(
    system="You are a a helpful, respectful and honest language and vision assistant. "
           "You are able to understand the visual content that the user provides, and "
           "assist the user with a variety of tasks using natural language.",
    roles=("user", "assistant"),
    version="llama_v3",
    messages=[],
    offset=0,
    sep="<|eot_id|>",
    sep_style=SeparatorStyle.LLAMA_3,
    tokenizer_id="/data1/Models/Meta-Llama-3-8B",  # meta-llama/Meta-Llama-3-8B-Instruct
    tokenizer=safe_load_tokenizer("/data1/Models/Meta-Llama-3-8B"),
    stop_token_ids=[128009],
)
...
if __name__ == "__main__":
    ...
    print('=============')
    conv = copy.deepcopy(conv_templates['llama_3_1'])
    conv.append_message(conv.roles[0], 'Hello. How Are you?')
    conv.append_message(conv.roles[1], None)
    prompt = conv.get_prompt()
    print(prompt)
    ...
    print('=============')
    conv = copy.deepcopy(conv_templates['llama_3_2'])
    conv.append_message(conv.roles[0], 'Hello. How Are you?')
    conv.append_message(conv.roles[1], None)
    prompt = conv.get_prompt()
    print(prompt)
